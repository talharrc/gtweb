...
export default function HowWeWork() {
  ...
  return (
    <section ref={processRef} className="py-16 px-6 relative overflow-hidden" style={{ background: 'linear-gradient(to bottom, transparent 0%, rgba(14,31,20,0.24) 50%, transparent 100%)' }}>
      ...
      <div className="absolute bottom-0 left-0 w-[380px] h-[380px] bg-[#2BFF6E]/[0.045] blur-[130px] rounded-full pointer-events-none" />
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-12 gap-6">
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.7 }}>
            ...
            <h2 className="display-sora text-4xl sm:text-5xl md:text-6xl mb-6">
              ...
              <span className="pill-word-accent text-white text-2xl sm:text-3xl md:text-4xl mt-2 inline-block" style={{ background: 'linear-gradient(135deg, #2BFF6E, #7CFFC2)' }}>Work.</span>
            </h2>
          </motion.div>
              ...
              >
                {isIcy ? (
                  <div className="w-full h-full p-7 rounded-3xl icy-card group">
                    <div className="absolute right-5 bottom-4 font-black select-none pointer-events-none leading-none z-0" style={{ fontSize: '100px', color: 'rgba(43,255,110,0.05)', fontFamily: 'var(--font-display)' }}>{step.num}</div>
                    <div className="flex items-start justify-between mb-6 relative z-10">
                      <div className="w-12 h-12 rounded-xl flex items-center justify-center glass-icon-3d-light float-idle text-[#2BFF6E] group-hover:scale-105 transition-transform duration-300 [&>svg]:relative [&>svg]:z-10" style={{ animationDelay: `${i * 0.4}s` }}>
                        <StepIcon className="w-5 h-5" />
                      </div>
                      <span className="text-[11px] font-mono text-[#16C95A] font-bold bg-[#2BFF6E]/10 px-2.5 py-1 rounded-full">{step.num}</span>
                    </div>
                  ...
                  <SpotlightCard
                    ...
                    style={{ borderColor: isIlluminated ? '#7CFFC2' : 'rgba(255,255,255,0.08)' }}
                  >
                    ...
                    <div className="flex items-start justify-between mb-6 relative z-10">
                      <div className="w-12 h-12 rounded-xl flex items-center justify-center glass-icon-3d float-idle text-[#EFFFF5] group-hover:scale-105 transition-transform duration-300 [&>svg]:relative [&>svg]:z-10" style={{ animationDelay: `${i * 0.4}s` }}>
                        <StepIcon className="w-5 h-5" />
                      </div>
