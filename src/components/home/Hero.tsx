...
export default function Hero({ isMobile }: HeroProps) {
  ...
  return (
    <section ref={heroRef} className="relative min-h-[78vh] sm:min-h-[88vh] flex flex-col items-center justify-center pt-28 pb-10 sm:pb-16 overflow-hidden">
      <div className="absolute inset-0 z-0 select-none overflow-hidden" style={{ background: 'radial-gradient(ellipse 90% 70% at 50% 78%, #0AD65C 0%, #0A5C33 32%, #071A0F 62%, #050806 100%)' }}>
        <div
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-[#050806] via-[#050806]/40 to-transparent pointer-events-none" style={{ height: '55%' }} />
        <div className="absolute inset-x-0 bottom-0 h-1/4 bg-gradient-to-t from-[#050806] to-transparent pointer-events-none" />
        {/* Vivid atmospheric brand blob glows */}
        <motion.div style={{ y: yBlob1 }} className="absolute top-1/3 left-1/4 -translate-y-1/2 -translate-x-1/2 w-[560px] h-[560px] bg-[#2BFF6E]/25 blur-[130px] rounded-full pointer-events-none" />
        <motion.div style={{ y: yBlob2 }} className="absolute top-1/2 right-1/4 -translate-y-1/2 translate-x-1/2 w-[420px] h-[420px] bg-[#7CFFC2]/22 blur-[110px] rounded-full pointer-events-none" />
        <div className="absolute bottom-[8%] left-1/2 -translate-x-1/2 w-[640px] h-[340px] bg-[#5CFF95]/35 blur-[90px] rounded-full pointer-events-none" />
      ...
      <motion.div style={{ y: contentY, opacity: contentOpacity }} className="max-w-5xl mx-auto px-4 sm:px-6 text-center relative z-10 pt-8 sm:pt-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}>
          <div className="eyebrow-badge rounded-full px-4 sm:px-5 py-2 sm:py-2.5 mb-12 max-w-[92vw]">
            <span className="w-1.5 h-1.5 rounded-full bg-[#7CFFC2] flex-shrink-0" />
        ...
        <h1 className="display-sora text-depth-dark text-[2.4rem] sm:text-[3.6rem] md:text-[5.8rem] mb-4" style={{ fontSize: 'clamp(2.6rem, 8vw, 6.5rem)' }}>
          ...
          <span className="display-sora typewriter-container block min-h-[1.2em] mt-3 pb-1 overflow-hidden whitespace-nowrap" style={{ filter: 'drop-shadow(0 8px 30px rgba(43,255,110,0.25))', fontSize: 'clamp(1.4rem, 5vw, 4.5rem)' }}>
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2, ease: [0.16, 1, 0.3, 1] }} className="text-sm sm:text-lg text-white/70 max-w-2xl mx-auto px-2 mb-10 leading-relaxed font-sans">
