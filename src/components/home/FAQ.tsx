...
export default function FAQ() {
  ...
  return (
    <section className="py-10 px-6 relative overflow-hidden" style={{ background: 'linear-gradient(to bottom, transparent 0%, rgba(11,36,112,0.20) 60%, transparent 100%)' }}>
      <div className="absolute top-0 right-1/4 w-[350px] h-[350px] bg-[#0059FF]/5 blur-[120px] rounded-full pointer-events-none" />
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-8 gap-6">
          <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.7 }}>
            ...
            <h2 className="display-sora text-4xl sm:text-5xl md:text-6xl mb-6">
              ...
              <span className="pill-word-accent text-white text-2xl sm:text-3xl md:text-4xl mt-2 inline-block" style={{ background: 'linear-gradient(135deg, #0059FF, #00C2FF)' }}>Questions.</span>
            </h2>
          </motion.div>
            ...
            return (
              <motion.div
                ...
                style={{
                  ...
                  border: `1px solid ${isActive ? 'rgba(0,89,255,0.55)' : 'rgba(0,194,255,0.22)'}`,
              ...
              >
                ...
                >
                  <FaqIcon className="w-4 h-4 flex-shrink-0 transition-transform duration-200" style={{ color: '#0059FF', transform: isActive ? 'scale(1.15)' : 'scale(1)' }} />
                  <span className="text-sm font-semibold flex-1 leading-snug" style={{ color: isActive ? '#030923' : '#334155' }}>{faq.q}</span>
                  <ChevronRight className="w-4 h-4 flex-shrink-0 transition-transform duration-200" style={{ transform: isActive ? 'rotate(90deg)' : 'none', color: isActive ? '#0059FF' : 'rgba(0,89,255,0.35)' }} />
                </button>
                <AnimatePresence>
                  {isActive && (
                    <motion.div
