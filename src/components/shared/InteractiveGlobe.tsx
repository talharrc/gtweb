...
const CLIENT_COUNTRIES: CountryNode[] = [
  { name: 'USA', lat: 37.0902, lon: -95.7129, color: '#7CFFC2' }, // Center of USA
  { name: 'UK', lat: 55.3781, lon: -3.4360, color: '#2BFF6E' },
  { name: 'Pakistan', lat: 29.9490, lon: 69.3451, color: '#7CFFC2' },
  { name: 'Saudi Arabia', lat: 23.8859, lon: 45.0792, color: '#16C95A' },
  { name: 'India', lat: 20.5937, lon: 78.9629, color: '#7CFFC2' },
];

const DHAKA: CountryNode = { name: 'Dhaka', lat: 23.6850, lon: 90.3563, color: '#2BFF6E' };
...
export default function InteractiveGlobe() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      glowGrad.addColorStop(0, 'rgba(124,255,194, 0.08)');
      glowGrad.addColorStop(0.5, 'rgba(43,255,110, 0.04)');
      ...
      clientNodes.forEach(c => {
        ...
        if (isDhakaFront || isClientFront) {
          ...
          ctx.strokeStyle = `rgba(124,255,194, ${arcAlpha})`;
          ctx.lineWidth = 1.2;
          ctx.stroke();
          ...
          if (ptRotZ > 0) {
            ctx.fillStyle = `rgba(43,255,110, ${(1 - t) * 0.95})`; // Cobalt signal particles fading out as they arrive
            ctx.beginPath();
            ctx.arc(px2d, py2d, 2.5, 0, Math.PI * 2);
            ctx.fill();

            // Glow flare
            ctx.fillStyle = `rgba(43,255,110, ${(1 - t) * 0.25})`;
            ctx.beginPath();
      ...
      if (dRotZ2 > 0) {
        ...
        ctx.fillStyle = 'rgba(43,255,110, 0.2)';
        ctx.beginPath();
        ctx.arc(dx2d, dy2d, pulseSize + 4, 0, Math.PI * 2);
        ctx.fill();

        ctx.fillStyle = '#2BFF6E'; // Core
        ctx.beginPath();
      ...
      clientNodes.forEach(c => {
        ...
        if (cRotZ2 > -radius * 0.1) {
          ...
          ctx.fillStyle = isHovered ? 'rgba(124,255,194, 0.4)' : 'rgba(124,255,194, 0.25)';
          ctx.beginPath();
          ctx.arc(cx2d, cy2d, pulseSize + 3, 0, Math.PI * 2);
          ctx.fill();

          ctx.fillStyle = '#7CFFC2'; // Core
          ctx.beginPath();
  ...
  return (
    <div className="relative w-full h-full flex flex-col items-center justify-center">
      ...
      />
      {hoveredNode && (
        <div className="absolute bottom-4 bg-[#050806]/90 backdrop-blur-md px-4 py-2 rounded-xl border border-white/10 text-xs font-mono font-semibold text-white pointer-events-none shadow-lg animate-fadeIn">
          Connected Node: <span className="text-[#7CFFC2]">{hoveredNode}</span>
        </div>
      )}
    </div>
  );
