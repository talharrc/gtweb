// Standalone, dependency-light serverless function for builder login — same
// pattern as api/auth/admin.ts / api/auth/client.ts, deliberately NOT part of
// api/index.ts's monolithic Express app (found unresponsive in production).
...
export default function handler(req: any, res: any) {
  ...
  if (payload?.type !== "invite" || payload?.role !== "builder") {
    res.statusCode = 401;
    res.setHeader("Content-Type", "application/json");
    res.end(JSON.stringify({ error: "This pass is not valid for builder access." }));
    return;
  }

  const token = jwt.sign(
    { role: "builder", email: payload.email, projectId: payload.projectId ?? null, projectName: payload.projectName ?? null },
    JWT_SECRET,
    { expiresIn: "7d" }
  );
